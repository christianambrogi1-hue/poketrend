 ...
 * history: array [{date:'YYYY-MM-DD', trend:number, est?:true}] ordinato crescente.
 * Ritorna null se non esiste un punto abbastanza vecchio entro la tolleranza.
 *
 * I punti marcati est sono stime ricavate dalle medie Cardmarket (backfill):
 * servono a dare forma al grafico, non a produrre una percentuale. Vengono
 * scartati qui, cosi' nessun numero pubblicato puo' nascere da una stima
 * travestita da rilevazione.
 */
export function trendFromHistory(history, days, today, tolerance = null) {
  if (!Array.isArray(history)) return null;
  const real = history.filter((p) => p && !p.est && p.trend > 0);
