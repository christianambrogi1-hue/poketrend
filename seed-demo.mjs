...
// Guardia: la demo sovrascrive cards.ndjson, prices.ndjson e state.json. Se li
// trova con dentro dati veri, si ferma. Senza questo controllo un "npm run demo"
// lanciato per curiosita' dopo il bootstrap cancella settimane di raccolta, e
// lo storico non si recupera all'indietro.
const marker = await fs.readFile(path.join(DATA, 'DEMO'), 'utf8').catch(() => null);
const esistente = await fs.readFile(path.join(DATA, 'prices.ndjson'), 'utf8').catch(() => null);
if (esistente && !marker && !process.argv.includes('--force')) {
  console.error('In data/ ci sono prezzi veri e non il marcatore DEMO: mi fermo.');
  console.error('La demo li sovrascriverebbe, e lo storico raccolto non si recupera.');
  console.error('Se vuoi davvero farlo: node scripts/seed-demo.mjs --force');
  process.exit(1);
}
